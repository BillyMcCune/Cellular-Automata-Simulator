...
/**
 * @author Billy McCune The ConfigReader class is responsible for reading and parsing XML
 * configuration files for the simulation. It supports different ways of defining the initial grid,
 * parameters, and accepted states.
 */
public class ConfigReader {
   ...
   *
   * @param stateCounts a map where the key is the state and the value is the number of cells in
   *                    that state.
   * @param totalCells  the total number of cells in the grid.
   * @return a shuffled list of cell state integers.
   * @throws IllegalStateException if the total number of states does not match the total cell
   *                               count.
   */
  private List<Integer> generateRandomizedStateList(Map<Integer, Integer> stateCounts,
      int totalCells) {
  ...
  /**
   * Parses the {@code initialStates} element from the XML and maps each state to its specified
   * count.
   *
   * @param root           the root XML element.
   ...
   */
  private Map<Integer, Integer> parseInitialStates(Element root, Set<Integer> acceptedStates,
      int totalCells) {
    ...
    if (!stateCounts.containsKey(0)) {
      ...
      if (specifiedSum != totalCells) {
        throw new IllegalArgumentException(
            "error-specifiedSumDoesNotEqualTotalSum" + "," + totalCells);
      }
    }
    return stateCounts;
  }
  ...
  /**
   * Parses the {@code initialProportions} element from the XML and calculates cell counts based on
   * the specified proportions.
   *
   * @param root           the root XML element.
   * @param acceptedStates the set of accepted cell states.
   * @param totalCells     the total number of cells in the grid.
   * @return a map where the key is the cell state and the value is the calculated count for that
   * state.
   * @throws IllegalArgumentException if any proportion values are invalid or the total proportions
   *                                  exceed 100%.
   * @throws IllegalStateException    if the total assigned cell count does not match the grid
   *                                  size.
   */
  private Map<Integer, Integer> parseInitialProportions(Element root, Set<Integer> acceptedStates,
      int totalCells) {
  ...
  /**
   * Parses the grid defined in the XML file from the {@code <initialCells>} format. Each row
   * contains multiple {@code <cell>} elements, which are converted to {@code CellRecord} objects.
   *
   * @param root the root XML element.
   * @return a 2D list of {@code CellRecord} representing the grid.
      ...
      for (int j = 0; j < cellNodes.getLength(); j++) {
        ...
        if (cellNode.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (stateStr == null || stateStr.isEmpty()) {
            throw new IllegalArgumentException("error-missingCellState" + "," + i + "," + j);
          }
          int state;
          try {
            ...
            throw new IllegalArgumentException(
                "error-invalidCellState" + "," + i + "," + j + "," + stateStr);
          }
          ...
          for (int k = 0; k < cellElement.getAttributes().getLength(); k++) {
            ...
            if (!attrName.equals("state")) {
              ...
              try {
                ...
                throw new IllegalArgumentException(
                    "error-InvalidCellState" + "," + i + "," + j + "," + attrName);
              }
            }
          }
  ...
  /**
   * Parses accepted states from a single {@code <acceptedStates>} element containing a
   * space‐separated list.
   *
   * @param root the root XML element.
   * @return a set of accepted state integers.
   * @throws IllegalArgumentException if the accepted states element is missing or contains invalid
   *                                  values.
   */
